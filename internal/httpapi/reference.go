...
var routeDocs = []reference.Route{
	...
	// --- installation -----------------------------------------------------
	{Method: "GET", Path: "/api/v1/adapters", Group: "Installation", Summary: "The adapters configured here and what they can currently do — live capabilities, not stored configuration. Names which credentials are set, never their values. `pending_restart` marks one saved since Pando started, which is not yet what runs; `restart_needed` says any is.", Verb: string(authz.InstallView)},
	{Method: "GET", Path: "/api/v1/adapters/kinds", Group: "Installation", Summary: "The kinds of adapter this build of Pando can run, and the settings each takes — which are credentials (write-only, stored encrypted), which are required, and the default each takes when left empty or an example.", Verb: string(authz.InstallView)},
	...
	{Method: "POST", Path: "/api/v1/ai/access/draft", Group: "Installation", Summary: "Draft a custom role and a group from a description, with verbs from the catalog only. Body: `description`. A draft: create it with POST /roles and POST /groups (R-343).", Verb: string(authz.InstallUsersManage)},
	{Method: "POST", Path: "/api/v1/ai/policy/draft", Group: "Installation", Summary: "Propose host policy from a description: the document as it would be saved, and each change. Fields fixed in the startup configuration are declined, naming where. Body: `description`. Save it with PUT /policy (R-344).", Verb: string(authz.InstallPolicyManage)},
