...
| `POST /api/v1/ai/access/draft` | `install.users.manage` | Draft a custom role and a group from a description, with verbs from the catalog only. Body: `description`. A draft: create it with POST /roles and POST /groups (R-343). |
| `POST /api/v1/ai/policy/draft` | `install.policy.manage` | Propose host policy from a description: the document as it would be saved, and each change. Fields fixed in the startup configuration are declined, naming where. Body: `description`. Save it with PUT /policy (R-344). |
