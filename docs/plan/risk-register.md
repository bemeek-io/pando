...
| No install-level authorization exists (O-17) | 8 | Six endpoints are gated by "are you signed in" and nothing else, because there is nothing to gate them with. A user with no grants can suspend the administrator and lock the install out — demonstrated on the shipped stack. The per-app model is sound; there is simply no install scope | **Open, blocking** |
