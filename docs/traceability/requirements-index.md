...
|---|---:|---:|
| Requirements | 223 | — |
| Specified in a design doc | 169 | 75% |
| Assigned to a phase | 137 | 61% |
| Covered by a named test | 124 | 55% |
...
| **R-020** | D | Nothing lives in the repo. | 3. Core Invariants | 01, 02, 03, 07, 09 | 02, 06, 10 | `TestR020_AComposeBuildIsResolvedIntoTheSpec`, `TestR020_APinnedSpecCanBeExported`, `TestR020_AReadCannotLeaveTheCheckout`, `TestR020_AddingAVolumeWritesARevisionRatherThanCreatingStorage`, `TestR020_AnEditedPlanIsTheOneThatRuns`, `TestR020_ScreeningNeedsAReadableRepository`, `TestR020_SpecCarriesNoSecretValues`, `TestR020_TheViewCannotReachOutsideTheSource`, `TestR020_ToolCallsThatLeaveTheCheckoutAreRefused` |
| **R-021** | D | Pando fills declared slots; it never invents topology. | 3. Core Invariants | 01, 09 | 06 | `TestR021_APathAmendmentMustPointInsideTheRepository`, `TestR021_SlotsComeFromWhatTheRepoDeclares` |
...
| **R-027** | D | Authorization decisions, the audit log, the state store, and the identity assertion path live… | 3. Core Invariants | 00, 02, 03, 06, 09 | 00, 08, 11 | `TestR027_AnOwningRoleCanUndoTheRevoke`, `TestR027_ApplicationRoleDoesNotOwnTheSchema`, `TestR027_AuditLogIsNotRewritable`, `TestR027_AuditRemainsImmutableAcrossRestarts` |
| **R-028** | D | Pando observes and reports; it does not remediate the app. | 3. Core Invariants | 03, 05, 08, 09 | 08 | `TestR028_AScreenerCannotForgeAWarningCode` |
...
| **R-097** | D | A trial run in throwaway isolation is part of detection. | 7.2 Detection | 01, 03, 07, 09 | 06 | `TestR097_ATrialRunObservesTheBoundPort`, `TestR097_AWatchedPortAnswersTheQuestionInsteadOfAPerson`, `TestR097_ObservedPortsAreMarkedObservedNotFramework`, `TestR097_TheTrialRunnerIsNilWhenNoRuntimeIsConfigured` |
| **R-098** | D | The user reviews the proposal, then it pins. | 7.2 Detection | 09 | — | `TestR098_AScreeningCannotFloodTheReview` |
| **R-099** | D | Compose constructs incompatible with the boundary are rejected or rewritten, with the reason… | 7.2 Detection | 00, 01 | 06 | `TestR099_ABlockedProposalIsNotScreened`, `TestR099_ACandidateThatImportedNothingIsNotAdopted`, `TestR099_APublishedHostPortIsRewrittenNotHonored`, `TestR099_ARejectedComposeFileDoesNotSilentlyBecomeABuildpackGuess`, `TestR099_ConstructsThatBreakTheBoundaryAreRejectedWithReasons` |
...
| **R-102** | D | Ask, never guess. | 7.3 When detection cannot decide | 03, 04, 08, 09 | 08 | `TestR102_ACloseCallBecomesAQuestion`, `TestR102_AnsweringTheTieBreakAdoptsThatReading` |
| **R-103** | D | The number of questions is the product metric. | 7.3 When detection cannot decide | 01, 08, 09 | 06, 08, 11 | — |
| **R-104** | D | Questions are blockers; everything else is configuration. | 7.3 When detection cannot decide | 03, 08, 09 | 08 | `TestR104_AFrameworkPortIsADefaultRatherThanAQuestion`, `TestR104_AStartCommandFromTheBuildPlanIsNotAsked`, `TestR104_TheProposalCarriesTheInstallsOwnAnswers` |
...
| **R-106** | D | AI assistance is optional supporting functionality, never required. | 7.3 When detection cannot decide | 03, 09 | 11 | `TestR106_AMonorepoEntrypointIsWhatScreeningIsFor` |
| **R-107** | D | The correct failure: a repo needs Postgres and never mentions it anywhere — no compose… | 7.3 When detection cannot decide | 01, 09 | 06 | `TestR107_ACrashDoesNotInventASlotTheRepoNeverDeclared`, `TestR107_ACrashingTrialReturnsItsLog` |
| **R-310** | D | Screening reviews Pando's own answer; it is not a second detector. | 7.4 Screening a deployment plan | 09 | 11 | `TestR310_AScreeningReadsTheRepositoryThenSubmitsFindings`, `TestR310_ScreeningDoesNotReRankTheAuction` |
| **R-311** | D | Screening amends the spec and nothing else. | 7.4 Screening a deployment plan | 09 | — | `TestR311_AScreenerCannotOverwriteAValueAPersonSet`, `TestR311_AScreenerCannotWriteOverADependencyOrASecret`, `TestR311_AScreenerMayReplaceAValueDetectionInferred`, `TestR311_AnAmendmentLandsInTheDraftSpecAndIsAttributed` |
| **R-312** | D | The amendable surface is a closed set. | 7.4 Screening a deployment plan | 09 | 11 | `TestR312_ACommandLineIsHandedToAShell`, `TestR312_AnAmendmentKindThatIsNotInTheSetSaysNothing`, `TestR312_AnAmendmentMustSayWhichWorkloadWhenThereIsNoPrimary`, `TestR312_EachAmendmentKindLandsWhereItSays`, `TestR312_EachAmendmentKindRefusesWhatItMust`, `TestR312_RefusalsAreRecordedNeverSilent`, `TestR312_TheToolSchemaIsTheClosedSet` |
| **R-313** | D | An observation outranks a screening. | 7.4 Screening a deployment plan | 09 | 11 | `TestR313_AScreenedPortFillsWhatWasNotObserved`, `TestR313_AScreeningDoesNotOverruleTheTrialRun`, `TestR313_AnObservationOutranksAScreening` |
| **R-314** | D | Every amendment carries evidence and is attributed. | 7.4 Screening a deployment plan | 09 | 11 | `TestR314_AnAmendmentRestingOnNothingIsRefused`, `TestR314_AnAppliedAmendmentRecordsScreenedProvenance`, `TestR314_AnEmptyEvidenceEntryIsIgnoredRatherThanChecked`, `TestR314_EvidenceThatIsNotInTheRepositoryIsRefused`, `TestR314_WithoutAReadableRepositoryNothingIsApplied` |
| **R-315** | D | Screening never blocks and never fails a detection. | 7.4 Screening a deployment plan | 09 | 11 | `TestR315_AModelThatStopsWithoutFindingsIsAFailureNotACleanBill`, `TestR315_AProviderThatFailsDoesNotFailTheDetection`, `TestR315_ARefusalIsReportedAsAFailure`, `TestR315_AnUnconfiguredAdapterFailsRatherThanPretends`, `TestR315_EveryWayRunCanFailIsASkipWithAReason`, `TestR315_NoAdapterConfiguredLeavesTheProposalAlone` |
| **R-316** | P | Screening is on when an AI adapter is configured. | 7.4 Screening a deployment plan | 09 | 11 | `TestR316_AnAdapterTurnedOffForScreeningDoesNotScreen`, `TestR316_HostPolicyCanForbidAIScreening`, `TestR316_HostPolicyCanForbidScreeningInstallWide`, `TestR316_ScreeningIsOnWhenTheAdapterIsConfigured` |
| **R-317** | D | Screening sends repository contents to the adapter's provider, and records that it did. | 7.4 Screening a deployment plan | 09 | 11 | `TestR317_WhatWasReadIsRecordedInOrder` |
| **R-318** | D | A screener may answer Pando's own questions. | 7.4 Screening a deployment plan | 09 | 11 | `TestR318_AnAnswerIsHeldToTheSameEvidenceRule`, `TestR318_AnAnswerMayNotBeEmptyOrGivenTwice`, `TestR318_AnAnswerToAQuestionNobodyAskedIsRefused`, `TestR318_AnAnsweredQuestionStopsBeingAsked` |
| **R-319** | P | Screening is bounded by files read, bytes read, and wall clock, declared per adapter and | 7.4 Screening a deployment plan | 09 | 11 | `TestR319_RereadingAFileIsFree`, `TestR319_RunLowersTheBudgetToTheAdaptersAndSetsADeadline`, `TestR319_TheBudgetIsTheLowerOfWhatCoreAsksAndWhatTheAdapterWillDo`, `TestR319_TheByteBudgetIsEnforced`, `TestR319_TheFileBudgetIsEnforced` |
...
| **R-132** | D | Resolution is never silent. | 9. Slots and Services | 00, 01, 04, 05, 07, 09 | 02, 03, 06 | `TestR132_EveryUnfilledSlotIsNamed`, `TestR132_PlanningAPinnedSpecReportsWhatIsMissingAtPlanTime`, `TestR132_SlotUnfilledCarriesRemedyAndDetails`, `TestR132_UnfilledRequiredSlotBlocksDeploy` |
...
| **R-253** | D | Adapters are compiled in-tree. | 18. Adapters | 00, 03, 08, 09 | 00, 03 | `TestR253_EverySeededAdapterHasAnImplementation` |
| **R-254** | D | Every adapter advertises capabilities as data — `isolation_class`,… | 18. Adapters | 00, 03, 04, 05, 07, 09 | 03 | `TestR254_CapabilitiesAreHonest`, `TestR254_CapabilitiesAreReportedHonestly`, `TestR254_CapabilitiesReportPortModeOnly`, `TestR254_CapabilityUnsupportedBlocksDeploy`, `TestR254_TrialCapabilitiesAreDeclaredNotAssumed` |
...
| **R-258** | D | AI is the ninth adapter category. | 18. Adapters | 03, 09 | 11 | `TestR258_AIIsAnAdapterCategory`, `TestR258_AnAIAdapterConfiguredWithoutBeingMarkedDefaultIsStillFound`, `TestR258_NoAIAdapterIsAnOrdinaryOutcome` |
| **R-259** | D | An AI adapter declares which functions it performs as capabilities data. | 18. Adapters | 09 | 11 | `TestR259_CapabilitiesAreDataNotATypeAssertion` |
